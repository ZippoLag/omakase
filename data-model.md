...
- **Measured size on disk (2026-09-02)**: DB **318.2 MB** (full jmdict-eng, 218,577 words + 506k conjugation rows + FTS) + strokes **40 MB** (KanjiVG `-main`, 6,417 svgs) + kuromoji 41 MB → ~400 MB total. The trigram FTS index (~3× text) and the conjugation table are the main cost. The build takes ~18 s on a laptop (transform 2 s + inserts 14 s); the full-dictionary build previously hit an O(words × conjugations) summary computation in `data/build/index.ts`, now replaced with a Set lookup.
