/**
 * Kangxi radical number → radical character (the 214 traditional radicals).
 * Public-domain Unicode data. Used to display the classical radical of a
 * kanji as `radical 食 (184)` — the 184 is the radical *number* from
 * KANJIDIC2, mapped to its character here.
 */
export const KANGXI: Record<number, string> = {
...
/** Character for a Kangxi radical number, or the number itself if unknown. */
export function kangxiChar(number: number): string | null {
  return KANGXI[number] ?? null;
}